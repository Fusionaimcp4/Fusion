...
2. **Authenticate**: Click "Authorize" and provide either:
   - Bearer: your JWT
   - ApiKey: `ApiKey sk-fusion-...`
...
### 🌐 Endpoint Base and Paths
- Base URL (prod): `https://api.mcp4.ai`
- Paths are documented with `/api/...` (e.g., `/api/chat`, `/api/keys`)
- Full URLs generated by Swagger: `https://api.mcp4.ai/api/chat`, `https://api.mcp4.ai/api/keys`

### ⚠️ CORS Note
- Browser-based requests from origins other than the configured `FRONTEND_URL` are blocked by CORS
- Server-to-server requests (curl, Postman, backends) are not affected
